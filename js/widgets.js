var Widget = {
    ...
    loadCodeItemList: function (code_id, callback) {
        var url = "/index/index/codeItem?code_id=" + code_id;
        $.get(url, function (json) {
        	//  && json.code === 200
            if (callback != null) {
                callback(json.data);
            } else {
                layer.msg(json.msg || json.alertMsg || json.message || json.info || "服务器未返回数据", {icon: 2});
            }
        });
    }
};
...
Widget['textbox'] = {
    createInstance: function (cell) {
        ...
        var properties = {
            widgetType: 'textbox',
            id: field_code,
            index_code: field_code,
            index_name: field_code,
            save_type: 2,
            index_type: 'varchar',
            field_length: '200',
            placeholder: '输入提示'
